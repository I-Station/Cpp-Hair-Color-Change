...
// float Detector::preprocess(cv::Mat& img, cv::Mat& img_scale, float max_side, std::vector<bbox>& boxes)
// {
...
void Detector::box2faceobject( std::vector<bbox>& boxes, std::vector<FaceObject>& face_obj, float scale)

{

face_obj.resize(boxes.size());
for (int j = 0; j < boxes.size(); ++j) {
    // std::cout <<(boxes[j].point[0]._x / scale) << std::endl;
    face_obj[j].landmark[0].x = boxes[j].point[0]._x / scale;
    face_obj[j].landmark[1].x = boxes[j].point[1]._x / scale;
    face_obj[j].landmark[2].x = boxes[j].point[2]._x / scale;
    face_obj[j].landmark[3].x = boxes[j].point[3]._x / scale;
    face_obj[j].landmark[4].x = boxes[j].point[4]._x / scale;
    face_obj[j].landmark[0].y = boxes[j].point[0]._y / scale;
    face_obj[j].landmark[1].y = boxes[j].point[1]._y / scale;
    face_obj[j].landmark[2].y = boxes[j].point[2]._y / scale;
    face_obj[j].landmark[3].y = boxes[j].point[3]._y / scale;
    face_obj[j].landmark[4].y = boxes[j].point[4]._y / scale;
