...
public:
    ...
    void box2faceobject( std::vector<bbox>& boxes, std::vector<FaceObject>& face, float scale);

    // float preprocess(cv::Mat& img, cv::Mat& img_scale, float max_side, std::vector<bbox>& boxes);
